...
│   ├── epoll.cpp           # epoll 版：单进程 I/O 多路复用（Linux 专有）
│   ├── wrap.h              # 各种系统调用的封装声明
│   └── wrap.c              # 上面这些函数的实现
└── .vscode/                # 构建任务与 gdb 调试配置
```

四个程序各有自己的 `main()`，对应四个可执行文件，可以同时存在、互不干扰：
...
| `build/cpp_server_epoll` | `src/epoll.cpp` | 单进程单线程（epoll I/O 多路复用） | 8989 |

> 注意：select 版和 fork 版都监听 9999，**不能同时运行**，后启动的那个会报
> `Address already in use`。想同时跑，把其中一个的 `SRV_PORT` 改成别的端口。
> epoll 版用的是 8989，和上面三个都不冲突，可以直接起来做对比。
