...
两个程序各有自己的 `main()`，对应两个可执行文件，可以同时存在、互不干扰：
...
```bash
cd ~/projects/My_TcpServer
cmake -S . -B build -G Ninja
cmake --build build            # 两个程序一起编译
...
- `Ctrl+Shift+B`：构建（两个程序都会编译）
- `F5`：调试，启动时可以选择「调试 cpp_server (fork 版)」或「调试 cpp_server_thread (线程版)」
